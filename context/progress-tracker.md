...
- Implemented Responsive Pagination and Filtering in Team Management: Added client-side pagination (10 items per page) and Role/Department filters to the Team Management panel inside the Admin dashboard. Page indices automatically reset when filters or search queries change. Built a responsive UI showing large touch targets on mobile and detailed entry details with page controls on desktop. Verified full compilation via `npm run build`.
- Implemented Mobile Responsive Navigation Headers: Refactored the `AdminHeader` component to add a stateful hamburger menu toggle (using Lucide `Menu` and `X` icons) that reveals a collapsible slide-down navigation drawer on mobile viewports. Extracted the Stylist header into a dedicated `StylistHeader` client component, collapsing user details, change password trigger, and sign-out actions behind a mobile menu toggled directly by clicking the initials profile avatar circle. Verified complete layout responsiveness and compiled successfully via `npm run build`.
- Added Category & Department Filters in Stylist Performance Leaderboard: Refactored the `get_monthly_stylist_leaderboard` database function to return a unified table of stylist aggregates. Refactored `StylistLeaderboardCard` server component to execute a single RPC call, and updated `StylistLeaderboardClient` to render both Category and Department filters as responsive select dropdowns (All Categories, Ala Carte, Packages, Products and All Departments, Hair, Nails, Artistry & Lash), performing instant client-side sorting and filtering. Verified compilation via `npm run build`.
- Implemented Dropdown Date Range Selector: Replaced the standard monthly selector dropdown on both the Admin and Stylist dashboards with a structured dropdown supporting "This Month", "Last Month", and "Select Month Range". Choosing "Select Month Range" reveals custom From/To inputs and an "Apply" button to filter performance aggregates across arbitrary date ranges.
- Refactored Stylist Performance Leaderboard: Updated the `get_monthly_stylist_leaderboard` database function to filter for stylist profiles that have an assigned department (`HAIR`, `NAILS`, or `ARTISTRY_LASH`), excluding admin and unassigned/null profiles. Grouped transactions by the stylist's official profile name (`p.name`) instead of raw transaction aliases, displaying cleaner and consolidated leaderboard metrics on the Admin Dashboard.
- Implemented Self-Service Password Change for Stylists and Admins: Created a reusable `ChangePasswordDialog` modal component and added it to both the Stylist and Admin headers. Implemented the `changePasswordAction` Server Action, which re-authenticates the user with their current password via Supabase Auth before performing the update, and automatically logs the user out upon a successful change to force re-login for security. Verified the compilation via a successful full production build (`npm run build`).
